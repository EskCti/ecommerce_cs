...
- [x] 1.1 `infra:setup` Módulo Finance (~1h)
...
- [x] 2.1 `domain:vo` Finance VOs (~2h)
...
- [x] 3.1 `domain:entity` Receivable aggregate (~3h)
  - **Agent:** `Core Entity (C#)`
  - **Prompt:** "Receivable AR manual; AddAttachment; Settle(); child Attachment."
  - **Spec:** `receivable-management`

- [x] 3.2 `domain:service` ReceivableSettlementService (~2h)
...
- [x] 5.1 `domain:entity` Commission aggregate (~2h)
  - **Agent:** `Core Entity (C#)`
  - **Prompt:** "Commission AR linked SaleId, seller, amount, paid flag."
  - **Spec:** `commission-management`

- [x] 5.2 `domain:service` CommissionSettlementService (~2h)
  - **Agent:** `Core Domain Service (C#)`
...
- [x] 6.1 `app:usecase` Receivable CRUD + SettleReceivable (~4h)
  - **Agent:** `Core Use Case (C#)`
  - **Prompt:** "Create, Update, Delete, SettleReceivable manual receivables."
  - **Spec:** `receivable-management`

- [x] 6.2 `app:query` ListReceivablesQuery (~1h)
...
- [x] 7.1 `app:usecase` Payable CRUD + SettlePayable (~4h)
  - **Agent:** `Core Use Case (C#)`
  - **Prompt:** "CRUD despesas; SettlePayable RF-071."
  - **Spec:** `payable-management`

- [x] 7.2 `app:query` ListPurchasesQuery (~1h)
  - **Agent:** `Core Query CQRS (C#)`
...
- [x] 8.1 `app:usecase` PayCommission, PayCommissionsBatch (~3h)
  - **Agent:** `Core Use Case (C#)`
  - **Prompt:** "PayCommission individual e lote RF-075."
  - **Spec:** `commission-management`

- [x] 8.2 `app:query` ListCommissionsQuery, CashFlowQuery (~3h)
  - **Agent:** `Core Query CQRS (C#)`
...
- [x] 9.1 `app:usecase` SaleCompletedHandler (~3h)
  - **Agent:** `Core Use Case (C#)`
...
- [x] 10.1 `infra:persistence` LegacyReceivableMapper + Receivable aggregate (~8h)
...
- [x] 10.2 `infra:persistence` LegacyPayableMapper + LegacyCommissionMapper (~6h)
  - **Agent:** `Backend Data (C#)`
...
- [x] 11.1 `interface:controller` FinanceReceivablesController (~2h)
  - **Agent:** `Backend Controller (C#)`
  - **Prompt:** "CRUD /api/finance/receivables; POST settle; attachments sub-routes."
  - **Spec:** `receivable-management`

- [x] 11.2 `interface:controller` FinancePayablesController (~2h)
  - **Agent:** `Backend Controller (C#)`
  - **Prompt:** "CRUD /api/finance/payables; purchases list; settle."
  - **Spec:** `payable-management`

- [x] 11.3 `interface:controller` FinanceCommissionsController + CashFlowController (~2h)
  - **Agent:** `Backend Controller (C#)`
...
- [x] 12.1 `interface:entity` + usecase + repository Finance Vue (~4h)
...
- [x] 14.1 Validar US-070: CRUD receber manual, baixa, anexos RF-070
- [x] 14.2 Validar US-071: despesas, compras, ProductPurchased RN-061
- [x] 14.3 Validar US-072: comissões baixa/lote, fluxo caixa RF-074
- [x] 14.4 Validar integração SaleCompleted/Cancelled com EP-006
- [x] 14.5 Desbloquear EP-009 Reporting queries sobre Finance
