...
- [x] 1.1 `infra:setup` Módulo StoreSettings (~1h)
...
- [x] 2.2 `domain:entity` StoreConfig aggregate (~2h)
  - **Agent:** `Core Entity (C#)`
...
- [x] 4.1 `domain:entity` CashRegisterTerminal aggregate (~2h)
  - **Agent:** `Core Entity (C#)`
...
- [x] 6.1 `infra:persistence` Legacy ACL Store Settings (~3h)
...
- [x] 7.1 `interface:controller` StoreSettingsController (~2h)
...
- [x] 10.1 `test:unit` StoreSettings domain + app (~2h)
  - **Agent:** `Unit Tests (C#)`
  - **Prompt:** "xUnit: StoreConfig, PaymentMethod, PaymentSurchargeCalculator, CashRegisterTerminal. Coverlet ≥95%."

- [x] 10.2 `test:e2e` Settings API (~2h)
  - **Agent:** `E2E Tests (C#)`
  - **Prompt:** "WebApplicationFactory: GET/PUT store-config; CRUD payment-method; tenant isolation 403."

## 11. Acceptance verification

- [x] 11.1 Validar US-030: config loja RF-027 completa, form Vue, tenant scope
- [x] 11.2 Validar US-031: formas pgto com acrescimo, caixas físicos, RN-047 calculator
- [x] 11.3 Confirmar EP-006 pode referenciar PaymentMethodId e CashRegisterTerminalId
