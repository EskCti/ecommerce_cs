...
- [x] 1.1 `infra:setup` Módulo Notifications (~1h)
...
- [x] 2.1 `domain:vo` Notification VOs (~2h)
...
- [x] 3.1 `app:usecase` SendDailyTenantDigestHandler (~3h)
...
- [x] 4.1 `infra:adapter` EnviameWhatsAppAdapter (~3h)
...
- [x] 4.2 `infra:adapter` LegacyDigestQueryAdapter (~2h)
  - **Agent:** `Backend Data (C#)`
...
- [x] 4.3 `infra:hosted` DailyDigestBackgroundService (~2h)
  - **Agent:** `Backend Data (C#)`
...
- [x] 5.1 `interface:controller` NotificationsController (~1h)
...
- [x] 7.1 Validar US-100: digest contém contas hoje, estoque baixo, cobrança
- [x] 7.2 Validar RN-070: máximo 1 envio/dia/tenant
- [x] 7.3 Validar RN-071: token tenant sobrescreve global
- [x] 7.4 Comparar mensagem amostra vs legado mensagem.php
