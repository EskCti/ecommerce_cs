...
- [ ] 1.1 `infra:setup` Módulo Platform (~1h)
...
- [ ] 2.1 `domain:entity` Company, PlatformConfig (~2h)
  - **Agent:** `Core Entity (C#)`
...
- [ ] 2.2 `domain:service` TrialExpirationPolicy (~2h)
...
- [ ] 3.1 `app:usecase` RegisterTrialCompanyUseCase (~2h)
...
- [ ] 4.1 `infra:persistence` LegacyCompanyAdapter (~2h)
...
- [ ] 5.1 `interface:controller` POST /api/platform/trial (~1h)
...
- [ ] 6.1 `interface:form-web` Modal cadastro trial Vue (~2h)
...
- [ ] 7.1 `domain:entity` Company, Contract (~2h)
  - **Agent:** `Core Entity (C#)`
...
- [ ] 8.1 `app:usecase` CreateCompany, UpdateCompany, SaveContract (~4h)
  - **Agent:** `Core Use Case (C#)`
  - **Prompt:** "CreateCompany, UpdateCompany, SaveContract use cases; SAS scope only."
  - **Spec:** `platform-company-management`, `tenant-contracts`

- [ ] 8.2 `app:query` ListCompaniesQuery (~1h)
...
- [ ] 9.1 `interface:controller` PlatformCompaniesController (~2h)
  - **Agent:** `Backend Controller (C#)`
...
- [ ] 11.1 `domain:service` TenantBillingPolicy, TenantSuspensionPolicy (~3h)
  - **Agent:** `Core Domain Service (C#)`
...
- [ ] 12.1 `test:unit` Platform domain + application (~2h)
  - **Agent:** `Unit Tests (C#)`
  - **Prompt:** "xUnit + Moq: TrialExpirationPolicy, RegisterTrialCompany, SuspendOverdue. Coverlet ≥95% Platform.Core/Application."

- [ ] 12.2 `test:e2e` Trial + company CRUD (~2h)
  - **Agent:** `E2E Tests (C#)`
  - **Prompt:** "WebApplicationFactory: POST /api/platform/trial; SAS CRUD company; 403 tenant user."

## 13. Acceptance verification

- [ ] 13.1 Validar US-020: trial cria company+admin, email único, data_pgto, form Vue
- [ ] 13.2 Validar US-021: CRUD SAS, contratos, listagem Vue
- [ ] 13.3 Validar US-022: cobrança, suspensão, desativação usuários, msg bloqueio
