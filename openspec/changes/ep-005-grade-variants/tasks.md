...
- [x] 1.1 `domain:entity` GradeVariant (~3h)
  - **Agent:** `Core Entity (C#)`
...
- [x] 1.2 `domain:service` GradeVariantSyncService (~3h)
...
- [x] 1.3 `domain:entity` Ajustar Product + GradeOption (~2h)
  - **Agent:** `Core Entity (C#)`
...
- [x] 2.1 `app:usecase` Estender ConfigureProductGradeUseCase (~3h)
...
- [x] 2.2 `app:usecase` ResolveGradeVariantUseCase (~2h)
  - **Agent:** `Core Use Case (C#)`
...
- [x] 3.1 `infra:persistence` Schema + LegacyGradeVariantMapper (~4h)
...
- [x] 3.2 `infra:persistence` Migração estoque 1D → Variant (~2h)
  - **Agent:** `Backend Data (C#)`
  - **Prompt:** "Bootstrap/migration: copiar GradeOption.Stock existente para GradeVariant 1:1 em produtos single-dimension."
  - **Spec:** `product-grades`

- [x] 3.3 `infra:persistence` StockLegacyAdapter variant stock (~3h)
  - **Agent:** `Backend Data (C#)`
...
- [x] 4.1 `interface:controller` Estender CatalogGradesController (~2h)
...
- [x] 6.3 `interface:form-web` Matriz Cor × Tamanho (~4h)
...
- [x] 7.1 `interface:form-web` PdvView picker 2 dimensões (~3h)
  - **Agent:** `Frontend Form (Vue)`
...
- [x] 8.1 `test:unit` GradeVariantSync + policies (~3h)
  - **Agent:** `Unit Tests (C#)`
...
- [x] 8.3 `test:unit-web` Grade matrix entity (~1h)
