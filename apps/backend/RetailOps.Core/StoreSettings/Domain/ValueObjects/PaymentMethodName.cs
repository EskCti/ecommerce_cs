...
public record PaymentMethodName
{
    ...
    public static Result<PaymentMethodName> Create(string value)
    {
        ...
        if (trimmed.Length < 2)
            return Result<PaymentMethodName>.Failure("Payment method name must be at least 2 characters long.");
    ...
    public static implicit operator string(PaymentMethodName paymentMethodName) => paymentMethodName.Value;
}